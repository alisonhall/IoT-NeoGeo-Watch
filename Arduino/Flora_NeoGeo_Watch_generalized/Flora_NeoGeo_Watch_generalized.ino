...
//desired destination:                              |
#define GEO_LAT                43.659682
#define GEO_LON               -79.502925
//--------------------------------------------------|
...
Adafruit_NeoPixel stripOne = Adafruit_NeoPixel(1, 8, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip = Adafruit_NeoPixel(NUM_RING_PIXELS, RING_PIN, NEO_GRB + NEO_KHZ800);

// Offset hours from gps time (UTC)
//const int offset = 1;   // Central European Time
const int offset = -4;  // Eastern Daylight Time (USA)
//const int offset = -5;  // Centrasl Daylight Time (USA)
...
int lastMin = NUM_RING_PIXELS;
int lastHour = NUM_RING_PIXELS;
int startLED = 0;
int startLEDlast = NUM_RING_PIXELS;
int lastCombined = 0;
int start = 0;
int mode = 2;
int lastDir = NUM_RING_PIXELS;
int dirLED_r = 0;
int dirLED_g = 0;
int dirLED_b = 255;
int compassReading;
...
// Pushbutton setup
int buttonPin = BUTTON_PIN;             // the number of the pushbutton pin
...
void setup()
{
  ...
  stripOne.begin();
  stripOne.show(); // Initialize all pixels to 'off'
...
//  // Will run code when data is being sent to the arduino
  while (Serial.available() > 0) { 
      ...
      if (received == 'M') {
        ...
        // Change target GPS location

        targetLat = Serial.parseFloat();
        targetLon = Serial.parseFloat();
        
      } else if (received == 'P') {
        // Change Power/(On or Off) of the NeoPixel Ring and the single NeoPixel
        int typeVal = Serial.parseInt();
        ...
        stripOne.setPixelColor(0, strip.Color(0, 0, 0)); //change RGB color value
        stripOne.show();
      }
  }
//
//  Serial.println("Target GPS," + targetLat + "," + targetLon);
  ...
  if (mode == 0) {
//    Serial.println("MODE 0: Clock");
    stripOne.setPixelColor(0, strip.Color(255, 0, 0)); //change RGB color value
    stripOne.show();
    clockMode();
  }

  if (mode == 1) {
//    Serial.println("MODE 1: Nav");
    stripOne.setPixelColor(0, strip.Color(0, 255, 0)); //change RGB color value
    stripOne.show();
    navMode();
  }

  if (mode == 2) {
//    Serial.println("MODE 2: Compass");
    stripOne.setPixelColor(0, strip.Color(0, 0, 255)); //change RGB color value
    stripOne.show();
    compassMode();
  }
}
...
void buttonCheck() {
  ...
  if (buttonState == LOW && lastButtonState == LOW) {
    if ((millis() - buttonHoldTime) > buttonHoldDelay) {
      ...
      if(mode == 2) {
        mode = 0;
        lastMin = NUM_RING_PIXELS;
        lastHour = NUM_RING_PIXELS;
...
void clockMode() {
  if (start == 1) {
    ...
    if (minTemp < 0) {
      ledMin = minTemp + NUM_RING_PIXELS;
    } 
    else {
      ledMin = minTemp;
    }
    ...
    if (hourTemp < 0) {
      ledHour = hourTemp + NUM_RING_PIXELS;
    } 
    else {
      ledHour = hourTemp;
    }
...
void compassDirection(int compassHeading) 
{
  ...
  for(int i = 1; i < NUM_RING_PIXELS; i++){
  ...
  if (tempDir > (NUM_RING_PIXELS - 1)) {
    ledDir = tempDir - NUM_RING_PIXELS;
  }
  ...
  if (mode == 1) {
    ...
    if (ledDir > (NUM_RING_PIXELS - 1)) {
      ledDir = ledDir - NUM_RING_PIXELS;
    }
  } 
  else {
    ledDir = ledDir + compassOffset;
